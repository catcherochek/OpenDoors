...
class ControllerExtensionModuleAdaptiveimoprt extends Controller {
	...
	protected function validate() {//проверка разрешений доступа
		$this->language->load('extension/module/adaptiveimport');

		if (!$this->user->hasPermission('modify', 'extension/module/adaptiveimport')) {
	...
	public function index() {
		...
		$this->language->load('extension/module/adaptiveimport');//загрузка €зыкового файла

		$this->load->model('extension/module/adaptiveimport');//загрузка модели по пути admin\model\extension\module\adaptiveimport.php
        $this->load->model('setting/store');//¬озможно загрузка переменных магазина по аналогии с Setting
		$this->load->model('setting/setting');//загрузка массива setting из Ѕƒ. admin/model/settimg/setting.php
		$this->load->model('localisation/language');//¬озможно €зыковые параметры €зыка
		
		//интегрирует файл vendors/excelport/excelport_openstock.xml в систему vqmod, котора€ измен€ет ебану кучу файлов перед run-time'ом в папке admin\model\extension\module\
		// 		$this->model_extension_module_excelport->openstock_integrate();// cм. файл vendors/excelport/excelport_openstock.xml
		
		
		//провер€ет удачно ли скопированы файлы
...
//добавл€ем заголовок к ответу  см. https://woo.zp.ua/opdoc/classes/method/id-2368/   и в system/library/response.php
